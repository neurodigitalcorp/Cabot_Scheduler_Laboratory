...
const ShiftTable: React.FC<ShiftTableProps> = ({ 
  ...
  const selectShift = (shift: ShiftType) => {
    if (selectedCell && isPlanningMode) {
        ...
        <tbody className="divide-y divide-slate-200">
          {staff.map((person) => (
            <tr key={person.id} className="group hover:bg-slate-50 transition-colors h-11">
              <td className="px-3 py-1 border-r border-slate-800 bg-white sticky left-0 z-10 group-hover:bg-indigo-50/30 shadow-[1px_0_3px_rgba(0,0,0,0.05)] w-[260px]">
                <div className="flex items-center justify-between gap-2 h-full">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {/* FOTO DEL EMPLEADO (CON ZOOM AL HOVER) */}
                    <div 
                      className="relative w-8 h-8 rounded-full border border-slate-300 bg-slate-100 flex-shrink-0 shadow-sm z-10"
                    >
                      {person.photo ? (
                        <img 
                          src={person.photo} 
                          alt={person.name} 
                          className="w-full h-full object-cover rounded-full transition-all duration-300 hover:scale-[3] hover:z-[100] hover:shadow-2xl cursor-zoom-in relative" 
                        />
                      ) : (
