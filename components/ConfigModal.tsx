...
const ConfigModal: React.FC<ConfigModalProps> = ({ 
  ...
  const generateEmailHTML = () => {
    ...
    const anioVal = currentYear;
    const daysCount = getDaysInMonth(anioVal, currentMonth);
    
    let tableRows = '';
    staff.forEach(person => {
      let cells = '';
      for (let d = 1; d <= daysCount; d++) {
        const dk = formatDateKey(anioVal, currentMonth, d);
  ...
  <p>
    A continuación, Cronograma de Turnos del Personal de Laboratorio para el mes de
    <strong>${mesNombre}</strong>, Año <strong>${anioVal}</strong>.
  </p>
  ...
  <!-- BLOQUE FLEX SOLO PARA LAS TABLAS -->
  <div style="margin-top:30px; display:flex; gap:30px; align-items:flex-start; flex-wrap:wrap;">

    <div>
      <p style="font-weight:bold; font-size:13px; margin-bottom:10px;">Nomenclatura de Turnos</p>
    ...
    <div>
      <p style="font-weight:bold; font-size:13px; margin-bottom:10px;">Estados y Ausencias</p>
      <table style="border-collapse: collapse; font-size:11px;">
        <tr>
          <td style="border:1px solid #ddd; padding:6px;"><b>P</b></td><td style="border:1px solid #ddd; padding:6px;">Permiso</td>
          <td style="border:1px solid #ddd; padding:6px;"><b>CD</b></td><td style="border:1px solid #ddd; padding:6px;">Calamidad</td>
        </tr>
        <tr>
          <td style="border:1px solid #ddd; padding:6px;"><b>C</b></td><td style="border:1px solid #ddd; padding:6px;">Compensado</td>
          <td style="border:1px solid #ddd; padding:6px;"><b>V</b></td><td style="border:1px solid #ddd; padding:6px;">Vacaciones</td>
        </tr>
        <tr>
  ...
  <div style="margin-top:40px; border-top:1px solid #eee; padding-top:20px;">
    <p style="font-style:italic; font-weight:bold;">
      <span style="color:#555;">Notification sent by </span>
      <span style="color:#f97316; font-size:18px;">Cabot Scheduler</span>
      <span style="color:#666;"> a Neuro Digitalverse App´s</span>
    </p>

    <p style="font-family:Courier New, monospace; font-weight:bold;">neuroDigital.</p>
    <p style="font-family:Courier New, monospace; font-style:italic; font-size:12px;">
      Decoding Tomorrow Through neuroDigitalverse.
    </p>

    <div style="margin-top:20px;">
      <p style="font-weight:bold; color:#dc2626;">Cabot Colombiana S.A.S</p>
      <p style="font-weight:bold;">Cartagena</p>
...
const callResendAPI = async (subject: string, html: string) => {
  ...
  const handleSendManualEmail = async () => {
    ...
    try {
      const subject = ` Cronograma Turnos Personal Laboratorio Cabot Colombiana S.A.S // — ${MONTHS[currentMonth]} ${currentYear}`;
